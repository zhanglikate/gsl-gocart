module seas_mod
  ...
  use gsd_chem_constants, only : kind_chem, pi=>con_pi
...
!
    ...
    real(kind=kind_chem), dimension(1,1,1,number_ss_bins) :: tc1
    real(kind=kind_chem), dimension(1,1,number_ss_bins) :: bems1
    real(kind=kind_chem), allocatable :: random_factor2(:)
...
!
            if(xland(i,j).lt.0.5)then
              ...
              tc(2)=1.e-30_kind_chem
              tc(3)=chem(i,kts,j,p_seas_2)*conver
              tc(4)=1.e-30_kind_chem
...
! ****************************************************************************
    ...
    REAL(kind=kind_chem),    INTENT(IN)    :: dxy(jmx), w10m(imx,jmx)
    REAL(kind=kind_chem),    INTENT(IN)    :: airmas(imx,jmx,lmx)
...
!!!  REAL(kind=kind_chem),    PARAMETER :: frh = 1.65
    ...
    REAL(kind=kind_chem) :: tc1(imx,jmx,lmx,nmx)
    REAL(kind=kind_chem), TARGET :: tcms(imx,jmx,lmx,nmx) ! tracer mass (kg; kgS for sulfur case)
    REAL(kind=kind_chem), TARGET :: tcgm(imx,jmx,lmx,nmx) ! g/m3
...
! REAL(kind=kind_chem) :: ch_ss(nmx,12)
    ...
    ! emissions (input)
    !-----------------------------------------------------------------------  
    REAL(kind=kind_chem) :: e_an(imx,jmx,2,nmx), e_bb(imx,jmx,nmx), &
...
!  REAL(kind=kind_chem), TARGET :: bems(imx,jmx,nmx), bdry(imx,jmx,nmx), bstl(imx,jmx,nmx)
!  REAL(kind=kind_chem), TARGET :: bwet(imx,jmx,nmx), bcnv(imx,jmx,nmx)!

!  ! integrated tendencies per process
!  REAL(kind=kind_chem), TARGET :: tems(imx,jmx,nmx), tstl(imx,jmx,nmx)
!  REAL(kind=kind_chem), TARGET :: tdry(imx,jmx,nmx), twet(imx,jmx,nmx), tcnv(imx,jmx,nmx)
    ...
    REAL(kind=kind_chem), TARGET :: ems_an(imx,jmx,nmx),    ems_bb(imx,jmx,nmx), ems_tp(imx,jmx)
    REAL(kind=kind_chem), TARGET :: ems_ac(imx,jmx,lmx,nmx)
    REAL(kind=kind_chem), TARGET :: ems_co(imx,jmx,nmx)
